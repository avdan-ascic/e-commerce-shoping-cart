...
import { update, readById, readAll } from "../../api/book-api";

const EditItem = ({ setAllItems }) => {
      ...
      .then((data) => {
        if (data.book) {
          setValues({
            ...
            reorderThreshold: data.book.reorderThreshold,
            stock: data.book.stock,
          });
          setStopOrderCheck(data.book.stopOrder);
          setBookImage(data.book.image);
        }
      })
  ...
  const validateForm = () => {
    ...
    if (!bookImage) {
      setValues({ ...values, error: "Please choose Image !" });
      return false;
    }
